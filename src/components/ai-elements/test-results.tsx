...
export type TestStatus = "passed" | "failed" | "skipped" | "running" | "warning";

interface TestResultsSummaryValue {
  passed: number;
  failed: number;
  skipped: number;
  total: number;
...
interface TestResultsContextType {
  summary?: TestResultsSummaryValue;
}
...
export const TestResultsProgress = ({
  ...
  const passedPercent = (summary.passed / denominator) * 100;
  const warningPercent = ((summary.warning || 0) / denominator) * 100;
  const failedPercent = (summary.failed / denominator) * 100;
...
const statusIcons: Record<TestStatus, ReactNode> = {
  ...
  warning: <CircleDotIcon className="size-4" />,
};
...
export const Test = ({
  ...
  return (
    <TestContext.Provider value={contextValue}>
      <div
        className={cn("flex flex-wrap items-center gap-2 px-4 py-2 text-sm", className)}
        {...props}
      >
        {children ?? (
          <>
...
export const TestErrorStack = ({
  ...
  <pre
    className={cn(
      "mt-2 overflow-auto whitespace-pre-wrap break-words font-mono text-red-600 text-xs dark:text-red-400",
      className
    )}
    {...props}
  >
