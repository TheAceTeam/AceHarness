...
interface ToastContextValue {
  toast: (type: Toast['type'], message: string) => number;
  dismissToast: (id: number) => void;
  updateToast: (id: number, type: Toast['type'], message: string) => void;
  clearToasts: () => void;
}
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const scheduleSuccessDismiss = useCallback((id: number) => {
  ...
  const iconMap = { success: 'check_circle', error: 'error', info: 'info', warning: 'warning', loading: 'progress_activity' };
  const colorMap = {
    success: 'bg-green-600 text-white',
    error: 'bg-destructive text-destructive-foreground',
    info: 'bg-primary text-primary-foreground',
    warning: 'bg-yellow-600 text-white',
    loading: 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900',
  };

  return (
    <ToastContext.Provider value={{ toast, dismissToast, updateToast, clearToasts }}>
      {children}
      <div className="fixed bottom-4 right-4 z-[400] pointer-events-auto flex flex-col gap-2 max-w-sm">
        ...
        {toasts.map((t) => (
          <div key={t.id}
            ...
            <span className={`material-symbols-outlined text-base mt-0.5 ${t.type === 'loading' ? 'animate-spin' : ''}`}>{iconMap[t.type]}</span>
