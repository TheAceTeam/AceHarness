import type { CollaborationWerewolfPlayer } from '@/lib/core/home-sidebar-state';
...
function countBoardRoles(roleDeck: CollaborationWerewolfPlayer['role'][]): Array<[CollaborationWerewolfPlayer['role'], number]> {
  const counts = new Map<CollaborationWerewolfPlayer['role'], number>();
  for (const role of roleDeck) {
...
export const WEREWOLF_ROLE_PROMPTS: Record<CollaborationWerewolfPlayer['role'], string> = {
  werewolf: '你属于狼人阵营。夜里和其他狼人统一刀人目标，白天要隐藏身份、制造合理怀疑、保护狼队友但不要过度绑定。可以在危险时选择自爆打断白天流程，但这会暴露身份并进入下一夜。警长竞选策略：狼队上警不是为了空聊，而是为了争夺预言家位、警徽位和白天主导权。通常需要有狼人承担悍跳预言家、报假验人、留假警徽流或以强势好人姿态搅乱站边；如果没有狼去抢预言家位，狼队往往会非常被动。与此同时，也不要机械地认为警下必须留狼或警上必须只上一狼，你需要综合考虑当前板子、警上人数、警下投票空间、悍跳收益、做身份空间以及狼坑是否会过早暴露，再决定自己留警上、退水还是打倒钩。',
  seer: '你是预言家。每夜查验一名玩家阵营，只得到狼人/好人的阵营结果。白天可以用隐晦方式传递验人信息，也可以在关键时刻起跳，但要考虑被狼人针对的风险。警长竞选策略：预言家通常应该积极上警争取警徽，上警后要报验人信息（告诉大家你查了谁、结果是什么），拿到警徽后要规划好警徽流（如果你死了警徽传给谁），这是预言家的核心操作。除非场上形势特殊（比如你判断悍跳狼很强需要藏一手），否则不要轻易退水。',
  witch: '你是女巫。你有一瓶解药和一瓶毒药，各只能使用一次。首夜可以自救。夜里会看到狼人袭击目标，再决定是否救人或毒人；白天不要随意暴露药水状态。用药原则：解药很珍贵，默认偏保守，不要因为有人首夜倒牌就机械救人。若无法确认刀口价值，通常优先考虑留药保护后续可能吃刀的预言家、警长真信息位或关键神职；只有在刀口明显是高价值神职、局势会因这一救产生巨大收益、或你自己首夜中刀必须自救时，才更适合果断开药。毒药同样不要随意交，要结合白天站边和夜间收益判断。警长竞选策略：女巫可以根据局势选择上警或不上警，如果预言家已经明确上警，你通常更适合低调待在警下保身份。',
  hunter: '你是猎人。你死亡时可以选择是否发动技能带走一名玩家；若已开枪则不能再次发动。白天要留意谁值得带走，也要避免过早暴露导致被利用。警长竞选策略：猎人可以上警争取话语权，拿到警徽后即使被刀也能开枪带人，是不错的选择。',
  idiot: '你是白痴。被白天投票放逐时会翻牌免死，但之后失去投票权。你可以用较松弛的方式扰动局势，但仍要帮助好人找狼人。',
  guard: '你是守卫。每夜守护一名玩家，通常不能连续两夜守同一人。守中狼人刀口可免死；请谨慎处理守护目标和白天信息释放。警长竞选策略：守卫可以考虑上警，拿到警徽能更好地保护关键位置。',
  villager: '你是普通村民。没有夜间能力，主要依靠发言、票型和阵营收益推理。白天要积极给出判断，帮助神职藏身份并逼出狼人的矛盾。警长竞选策略：村民也可以积极上警争取话语权，不要总是退水把机会让给别人，拿到警徽能帮助好人阵营归票。',
};
...
export function describeWerewolfBoardRoles(board: WerewolfLabBoard): string {
  return countBoardRoles(board.roleDeck)
    .map(([role, count]) => `${count} ${formatWerewolfBoardRole(role)}`)
    .join(' / ');
}

export function getWerewolfBoardAbsentRoles(board: WerewolfLabBoard): string[] {
  const present = new Set(board.roleDeck);
  const allRoles: CollaborationWerewolfPlayer['role'][] = ['seer', 'witch', 'hunter', 'idiot', 'guard', 'villager', 'werewolf'];
  return allRoles
    .filter((role) => !present.has(role))
    .map((role) => formatWerewolfBoardRole(role));
}

function formatWerewolfBoardRole(role: CollaborationWerewolfPlayer['role']): string {
  switch (role) {
    case 'werewolf':
      return '狼人';
    case 'seer':
