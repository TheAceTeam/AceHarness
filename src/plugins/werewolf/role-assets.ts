...
export const WEREWOLF_ROLE_ASSETS: Record<WerewolfRoleAssetKey, { col: number; row: number; label: string }> = {
  ...
  // The stitched atlas in public/images/werewolf.png places these cards in a
  // slightly different order than the original prompt description.
  guard: { col: 5, row: 0, label: '守卫' },
  dreamer: { col: 6, row: 0, label: '摄梦人' },
  knight: { col: 0, row: 1, label: '骑士' },
  bear: { col: 1, row: 1, label: '熊' },
  fox: { col: 2, row: 1, label: '狐狸' },
  crow: { col: 3, row: 1, label: '乌鸦' },
