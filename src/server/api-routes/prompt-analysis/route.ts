...
export async function POST(request: Request) {
  try {
    const { prompt, output, context, agentName } = await readJsonBody<any>(request, {});

    if (!prompt) {
      return jsonOk({ error: 'Prompt is required' }, { status: 400 });
    }
    ...
    return jsonOk({ error: 'Analysis failed' }, { status: 500 });
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!runId) {
      return jsonOk({ error: 'Run ID is required' }, { status: 400 });
    }
    ...
    try {
      ...
      return jsonOk({ error: 'Run not found' }, { status: 404 });
    }
