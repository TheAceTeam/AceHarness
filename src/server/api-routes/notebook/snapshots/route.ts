...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!file) return jsonOk({ error: '缺少 file 参数' }, { status: 400 });
    if (isBuiltinNotebookPath(file)) {
      return jsonOk({ error: '内置文档不支持快照' }, { status: 403 });
    }
    ...
    if (snapshotId) {
      ...
      if (!snapshot) {
        return jsonOk({ error: '快照不存在' }, { status: 404 });
      }
      return jsonOk({
        snapshot: {
          id: snapshot.id,
    ...
    return jsonOk({ error: error?.message || '获取快照列表失败' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if ('error' in access) {
      return jsonOk({ error: access.error }, { status: access.status });
    }
    ...
    return jsonOk({ error: error?.message || '创建快照失败' }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (!file || !snapshotId) {
      return jsonOk({ error: '缺少 file 或 snapshotId 参数' }, { status: 400 });
    }
    if (isBuiltinNotebookPath(file)) {
      return jsonOk({ error: '内置文档不支持快照' }, { status: 403 });
    }
    ...
    return jsonOk({ success: true, restoredSnapshotId: snapshot.id });
  } catch (error: any) {
    return jsonOk({ error: error?.message || '恢复快照失败' }, { status: 500 });
  }
}
