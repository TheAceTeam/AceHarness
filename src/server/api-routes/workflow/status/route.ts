...
function createWorkflowStatusStream(request: Request, configFile?: string | null, requestedRunId?: string | null) {
...
export async function GET(request: Request) {
  try {
    ...
    const requestedRunId = requestUrl(request).searchParams.get('runId');
    const live = requestUrl(request).searchParams.get('live') === '1';
    const compact = requestUrl(request).searchParams.get('compact') === '1';
