...
export async function POST(request: Request) {
  ...
  try {
    const body = await readJsonBody<SpecImportRequest>(request, {});
    if ((body.action || 'import-delta') !== 'import-delta') {
      return jsonOk({ error: '不支持的 spec import action' }, { status: 400 });
    }
    if (!body.runId) {
    ...
    if (!runState) {
      return jsonOk({ error: '运行状态不存在' }, { status: 404 });
    }
    if (body.configFile && runState.configFile !== body.configFile) {
      return jsonOk({ error: 'runId 与 configFile 不匹配' }, { status: 400 });
    }
    if (runState.persistMode !== 'repository' || runState.runSpecCoding?.persistMode !== 'repository') {
      return jsonOk({ error: '当前 run 不是持久化 Spec 模式' }, { status: 400 });
    }
    if (!runState.runSpecCoding) {
      return jsonOk({ error: '当前 run 没有运行态 SpecCoding 快照' }, { status: 400 });
    }
    ...
    if (!workflowName || !workingDirectory) {
      return jsonOk({ error: '缺少 workflowName 或 workingDirectory，无法定位 delta spec' }, { status: 400 });
    }
    ...
    if (!deltaSpec) {
      return jsonOk({ error: 'Delta spec 不存在，无法导入 workspace 修改' }, { status: 404 });
    }
