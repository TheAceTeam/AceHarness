...
export async function POST(request: Request) {
  try {
    const body = await readJsonBody<Partial<SpecMergeRequest>>(request, {});
    if (!body?.action) {
    ...
    if (body.action === 'preview') {
      return jsonOk(await previewMerge(body.runId, body.configFile));
    }
    if (body.action === 'apply') {
      return jsonOk(await applyMerge(body.runId, body.configFile, body.mergedHash));
    }

    return jsonOk({ error: '不支持的 action' }, { status: 400 });
  } catch (error: any) {
