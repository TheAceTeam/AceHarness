...
export async function GET(request: Request) {
  try {
    const status = requestUrl(request).searchParams.get('status');
    const runId = requestUrl(request).searchParams.get('runId');
    const configFile = requestUrl(request).searchParams.get('configFile');
    const limit = Math.max(1, Math.min(200, Number(requestUrl(request).searchParams.get('limit') || 50)));
    ...
    return jsonOk({ error: error?.message || '获取 Supervisor 消息失败' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!configFile && !runId) {
      return jsonOk({ error: '缺少 configFile 或 runId 参数' }, { status: 400 });
    }
    ...
    if (!title || !message) {
      return jsonOk({ error: '缺少标题或消息内容' }, { status: 400 });
    }
    ...
    return jsonOk({ error: error?.message || '创建 Supervisor 消息失败' }, { status: 400 });
  }
}
