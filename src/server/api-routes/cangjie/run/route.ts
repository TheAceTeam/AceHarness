...
export async function POST(req: Request) {
  ...
  try {
    const body = await readJsonBody<RunCangjieRequest>(req, { code: '' });
    const code = typeof body.code === 'string' ? body.code : '';
    if (!code.trim()) {
      return jsonError('代码不能为空', 400);
    }
    if (code.length > 200_000) {
      return jsonError('代码过长，无法运行', 400);
    }

    const cangjieHome = await detectCangjieHome({ userId: auth.id });
    if (!cangjieHome) {
      return jsonError('未检测到 CANGJIE_HOME，请先在环境变量中配置仓颉 SDK 根目录', 400);
    }

    const setupScript = resolve(cangjieHome, 'envsetup.sh');
    if (!isWindows() && !existsSync(setupScript)) {
      return jsonError(`未找到 envsetup.sh: ${setupScript}`, 400);
    }
    ...
    return jsonOk({
      success: false,
      stdout: '',
      stderr: errorMessage(error) || '运行失败',
      combinedOutput: errorMessage(error) || '运行失败',
      exitCode: null,
      error: errorMessage(error) || '运行失败',
    }, { status: 500 });
  } finally {
    if (outputPath) {
