...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!contentType.toLowerCase().includes('multipart/form-data')) {
      return workspaceRouteJsonError('请求必须是 multipart/form-data', 400);
    }
    ...
    if (files.length === 0) {
      return workspaceRouteJsonError('缺少上传文件', 400);
    }

    if (files.length > WORKSPACE_UPLOAD_FILE_COUNT_LIMIT) {
      return workspaceRouteJsonError(`上传文件数量超过 ${WORKSPACE_UPLOAD_FILE_COUNT_LIMIT} 个限制`, 413);
    }

    const conflict = typeof conflictValue === 'string' ? conflictValue : 'rename';
    if (conflict !== 'rename' && conflict !== 'error') {
      return workspaceRouteJsonError('conflict 参数不合法', 400);
    }
    ...
    if (isRemoteWorkspace(workspace)) {
      ...
      if (totalSize > WORKSPACE_UPLOAD_TOTAL_SIZE_LIMIT) {
        return workspaceRouteJsonError('上传总大小超过 200MB 限制', 413);
      }
      ...
      return jsonOk({ success: true, count: saved.length, files: saved });
    }
