...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!workspace || !action) {
      return workspaceRouteJsonError('缺少 workspace 或 action 参数', 400);
    }
    ...
    if (isRemoteWorkspace(workspace)) {
      ...
      switch (action) {
        ...
        case 'rename':
          ...
          return jsonOk({ success: true });
        case 'delete':
          if (!params.path) return workspaceRouteJsonError('不能删除 workspace 根目录', 400);
        ...
        case 'move':
          ...
          return jsonOk({ success: true });
        default:
          return workspaceRouteJsonError(`未知操作: ${action}`, 400);
      }
    }
    ...
    switch (action) {
      ...
      case 'delete': {
        ...
        if (fullPath === resolvedWorkspace) {
          return workspaceRouteJsonError('不能删除 workspace 根目录', 400);
        }
