import { errorMessage, jsonOk, requestUrl } from '@/server/api-route-runtime/request-utils';
...
export async function GET(request: Request) {
  ...
  const workingDirectory = (requestUrl(request).searchParams.get('cwd') || '').trim() || getWorkspaceRoot();
  ...
  try {
    ...
    return jsonOk({ engine, namespace, commands });
  } catch (error: any) {
    console.error(`[engine/commands] Failed to discover ${engine} commands:`, error);
    return jsonOk(
      { engine, namespace, commands: [], error: errorMessage(error) || 'Failed to discover commands' },
      { status: 500 },
    );
  }
}
