...
export async function GET(request: Request, { params }: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!canAccess(auth.id, session.createdBy)) {
      return jsonOk({ error: '无权访问该创建期会话' }, { status: 403 });
    }
...
export async function PUT(request: Request, { params }: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    ...
    return jsonOk({ error: error.message || '更新创建期会话失败' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!canAccess(auth.id, existing.createdBy)) {
      return jsonOk({ error: '无权删除该创建期会话' }, { status: 403 });
    }
    await deleteCreationSession(id);
    return jsonOk({ success: true });
  } catch (error: any) {
    return jsonOk({ error: error.message || '删除创建期会话失败' }, { status: 500 });
  }
}
