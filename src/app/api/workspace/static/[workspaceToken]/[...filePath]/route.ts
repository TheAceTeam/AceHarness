...
function rewriteRootAbsoluteUrl(value: string, workspaceToken: string): string {
  ...
  return `/api/workspace/static/${encodeURIComponent(workspaceToken)}/${encodeStaticPath(trimmed)}`;
}
...
export async function GET(
  ...
  { params }: { params: Promise<{ workspaceToken: string; filePath?: string[] }> },
) {
  try {
    ...
    if (ext === '.html' || ext === '.htm') {
      ...
      return new NextResponse(content, { headers: securityHeaders(contentType) });
    }
    if (ext === '.css') {
      const source = await fs.readFile(realPath, 'utf8');
