...
const RUNTIME_AGENT_ICON_PATHS: Record<string, string> = {
  ...
  codeagent: '/engines/codeagent.svg',
...
const STATIC_ENGINE_METADATA: Record<string, Omit<Engine, 'id' | 'status' | 'agentId'>> = {
  ...
  nga: {
    name: 'NGA',
    description: '适合既有 NGA 命令行开发流程。',
    features: ['OpenCode 兼容', '流式输出', '命令行集成'],
    endpoints: ['anthropic', 'openai'],
  },
  codeagent: {
    name: 'CodeAgent',
    description: '使用 ngagent ACP 启动并遵循新版权限语义。',
    features: ['Claude Code 兼容', 'ACPX 运行时', '流式输出'],
    endpoints: ['anthropic'],
  },
  codegenie: {
    name: 'CodeGenie',
  ...
  const handleSelectEngine = async (engineId: string) => {
    ...
    if (isEngineAvailable(normalizedEngineId) === false) {
      const hints: Record<string, string> = {
        ...
        codeagent: '请先安装 ngagent，并确认 ngagent 命令可用',
