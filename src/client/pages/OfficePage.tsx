...
import { WorkbenchHeader, type WorkbenchAction, type WorkbenchModeOption } from '@/components/ui/workbench-header';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { resolveDockviewTabPolicy, type DockviewTabPolicy } from '@/lib/navigation/dockview-tab-policy';
...
type WorkspaceStatusTab = 'office' | 'meeting' | 'workflow' | 'organization';
type TeamBuilderStep = 'goal' | 'clarify' | 'draft' | 'confirm';
type OfficeDockviewTargets = {
  workflows: { href: string; policy: DockviewTabPolicy };
  agents: { href: string; policy: DockviewTabPolicy };
  chats: { policy: DockviewTabPolicy };
  organization: { policy: DockviewTabPolicy };
};

const ZONES: Record<string, { label: string; color: string; brief: string; weight: number }> = {
  core: { label: 'CEO / Founder', color: '#6D5BD0', brief: 'Set Direction / Make Key Decisions', weight: 0 },
  product: { label: 'Product Lead', color: '#7C6F64', brief: 'Define Problems / Validate Value', weight: 10 },
  design: { label: 'Design Lead', color: '#6F8F72', brief: 'Design Experience / Create Solutions', weight: 20 },
  engineering: { label: 'Engineering Lead', color: '#B7794B', brief: 'Build Product / Ensure Quality', weight: 30 },
  growth: { label: 'Growth Lead', color: '#8B5CF6', brief: 'Drive Growth / Deliver Value', weight: 40 },
  operations: { label: 'Operations Lead', color: '#5E7E86', brief: 'Streamline Ops / Improve Efficiency', weight: 50 },
  quality: { label: 'Quality Lead', color: '#64748B', brief: 'Verify Risk / Keep Quality', weight: 60 },
  decision: { label: 'Decision Lead', color: '#8B5CF6', brief: 'Review / Decide / Align', weight: 70 },
  knowledge: { label: 'Knowledge Lead', color: '#5D8A7A', brief: 'Capture / Share Context', weight: 80 },
  generalist: { label: 'Generalist', color: '#6B7280', brief: 'Solve Problems / Support Anywhere', weight: 90 },
};
...
function AgentScopeDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl gap-5 rounded-lg p-0">
              ...
              {zones.map(([zone, zoneAgents]) => {
                ...
                return (
                  <button
                    key={zone}
                    type="button"
                    className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left text-xs font-bold transition ${checked ? 'border-primary/30 bg-accent text-accent-foreground' : 'bg-background hover:bg-muted'}`}
            ...
            <div className="max-h-[460px] space-y-2 overflow-y-auto pr-1">
              {filteredAgents.map((agent) => {
                ...
                return (
                  <label
                    key={agent.name}
                    className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition ${checked ? 'border-primary/30 bg-accent/80' : 'bg-background hover:bg-muted/60'}`}
                  >
...
function TeamComposer({
  ...
  return (
    <section className="mx-auto w-full max-w-5xl rounded-xl border border-border bg-card p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-primary/15 bg-accent text-accent-foreground">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <Button type="button" variant="outline" className="h-11" onClick={onFilterClick}>
            <SlidersHorizontal className="mr-2 h-4 w-4" />
            {filterActive ? '已筛选' : '筛选'}
          </Button>
          <Button type="button" variant="outline" className="h-11" onClick={onBuild} disabled={busy || (planReady ? false : !prompt.trim())}>
    ...
    <section className="mx-auto grid w-full max-w-6xl gap-4 md:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)]">
        ...
        <div className="mt-5 grid gap-3 text-sm">
          {OPC_CORE_ITEMS.map(([mark, label]) => (
            <div key={mark} className="flex items-center gap-3 rounded-xl border bg-background/70 p-3">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-purple-600 text-sm font-black text-white">{mark}</span>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {POWERED_BY_ITEMS.map((item) => {
          ...
          return (
            <div key={item.label} className="rounded-lg border bg-card p-4 shadow-none">
...
function TeamBuilderFlowModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[calc(100dvh-2rem)] max-w-5xl flex-col gap-0 overflow-hidden rounded-lg p-0">
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden md:grid-cols-[220px_minmax(0,1fr)]">
          <aside className="shrink-0 border-r bg-muted/30 p-5">
            <div className="space-y-3">
              {stepItems.map((item, index) => (
                <div
                  key={item.key}
                  className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-bold ${index <= stepIndex ? 'bg-background text-foreground shadow-none ring-1 ring-border' : 'text-muted-foreground'}`}
                >
                  <span className={`grid h-7 w-7 place-items-center rounded-full text-xs ${index <= stepIndex ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                    {index + 1}
                  </span>
                  {item.label}
                </div>
            ...
            <div className="mt-5 rounded-lg border bg-background p-3 text-xs text-muted-foreground">
          ...
          <section className="min-h-0 min-w-0 overflow-y-auto p-6">
            ...
            {step === 'clarify' && clarification ? (
              <div className="space-y-4">
                <div className="rounded-lg border bg-accent/70 p-4 text-sm text-accent-foreground">
                        ...
                        {question.options.map((option) => {
                          ...
                          return (
                            <button
                              key={option.id}
                              type="button"
                              className={`rounded-lg border p-3 text-left transition ${selected ? 'border-primary/35 bg-accent text-accent-foreground' : 'bg-background hover:bg-muted/60'}`}
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {sortPlanMembers(plan.members).map((member) => {
                    ...
                    return (
                      <div key={member.agentName} className="rounded-lg border p-4" style={{ ['--zone' as any]: zone.color }}>
                        <div className="flex items-center gap-3">
                          <SpriteAvatar
                ...
                {displayedGaps.length ? (
                  <div className="rounded-lg border border-amber-300/50 bg-amber-50/80 p-4 text-sm text-amber-950 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-[220px_minmax(0,1fr)]">
                      <div className="grid gap-2">
                        {displayedGaps.map((gap) => (
                          <button
                            key={gap.id}
                            type="button"
                            className={`rounded-lg border px-3 py-2 text-left text-xs font-bold transition ${selectedGap?.id === gap.id ? 'border-amber-500 bg-white text-amber-950 shadow-sm dark:bg-amber-500/20 dark:text-amber-50' : 'border-amber-300/60 bg-white/55 hover:bg-white dark:bg-transparent dark:hover:bg-amber-500/10'}`}
                      ...
                      {selectedGap ? (
                        <div className="rounded-lg border border-amber-300/60 bg-white/70 p-3 dark:bg-amber-500/10">
            ...
            {error ? (
              <div className="mt-4 rounded-lg border border-red-300/60 bg-red-50 p-3 text-sm text-red-700 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-200">
                {error}
              </div>
            ) : null}
          </section>
        ...
        <aside className={`org-editor ${manageMode ? 'org-editor-open' : ''}`} style={{ ['--zone' as any]: selectedZoneMeta.color }}>
          ...
          <div className="org-editor-section">
            ...
            <div className="org-editor-hint">
              {planReady ? '当前是组织草案，确认后写入 Agent YAML。' : '选择后会创建可确认的手动组织草案。'}
            </div>
          </div>
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-5">
      <div className="mb-4 flex items-end justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-normal">办公室实时状态</h2>
        </div>
        <StatusPill tone="success">{members.length} 人在线</StatusPill>
      </div>
      ...
      <div
        className={`office-floor office-floor-count-${renderedPresencePlans.length}`}
        style={{ minHeight: `${officeFloorHeightForTotal(renderedPresencePlans.length)}px` }}
      >
...
function MeetingRoomList({
  ...
  return (
    <aside className="meeting-room-list min-h-0 rounded-xl border border-border bg-background p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
...
function MeetingRoomTab({
  ...
  if (!room || !sessionId) {
    return (
      <div className="meeting-room-shell grid min-h-0 gap-4 rounded-xl border border-border bg-card p-4 md:grid-cols-[320px_minmax(0,1fr)]">
        <MeetingRoomList activeRoomId={room?.id} rooms={rooms} onSelectRoom={onSelectRoom} />
        <EmptyState
          icon={<MessageSquareText className="h-5 w-5" />}
          title="还没有打开会议室"
          description="在办公室里选择成员后点“私聊”或“拉人协作”，这里会直接进入对应的会议室对话。"
  ...
  return (
    <div className="meeting-room-shell grid min-h-0 gap-4 rounded-xl border border-border bg-card p-3 lg:grid-cols-[320px_minmax(0,1fr)]">
      <MeetingRoomList activeRoomId={room.id} rooms={rooms} onSelectRoom={onSelectRoom} />
...
function WorkflowStatusTab() {
  return (
    <div className="min-h-[560px] rounded-xl border border-border bg-card p-4">
      <EmptyState
        icon={<Workflow className="h-5 w-5" />}
        title="工作流协作"
        description="工作流启动后的协作议题、编队和人工确认会在这里集中呈现。"
        primaryAction={(
...
function WorkspaceStatusTabs({
  ...
  return (
    <Tabs value={activeTab} onValueChange={(value) => onTabChange(value as WorkspaceStatusTab)} className="space-y-4">
      <TabsList className="h-11 rounded-lg border border-border bg-card p-1 shadow-none">
        <TabsTrigger value="office" className="rounded-md px-4">
          <Building2 className="mr-2 h-4 w-4" />
          办公室
        </TabsTrigger>
        <TabsTrigger value="meeting" className="rounded-md px-4">
          <MessageSquareText className="mr-2 h-4 w-4" />
          会议室
        </TabsTrigger>
        <TabsTrigger value="workflow" className="rounded-md px-4">
          <Workflow className="mr-2 h-4 w-4" />
          工作流
        </TabsTrigger>
        <TabsTrigger value="organization" className="rounded-md px-4">
...
function OfficeStation({
  ...
  return (
    <div
      className={`station station-${activity} station-presence-${presence} station-layout-${layout} station-total-${total} ${selected ? 'station-selected' : ''}`}
      role="button"
      tabIndex={0}
...
function OfficeInspector({
  ...
  members: OfficeMember[];
  plan: OfficeTeamPlan | null;
  activeTab: WorkspaceStatusTab;
  activeRoom: CollaborationRoomRecord | null;
  recentRooms: CollaborationRoomRecord[];
  dockviewTargets: OfficeDockviewTargets;
  busy: boolean;
  onConfigureTeam: () => void;
  onApplyPlan: () => void;
  onOpenDirectRoom: (member: OfficeMember) => void;
  onOpenGroupRoom: (members: OfficeMember[]) => void;
  onSetTab: (tab: WorkspaceStatusTab) => void;
}) {
  const leadMember = members[0] || null;
  const missingZones = plan?.missingZones || [];

  return (
    <aside className="office-inspector flex min-h-0 flex-col gap-3">
      <DataCard>
        <DataCardHeader>
          <div>
            <DataCardTitle>Workspace</DataCardTitle>
            <DataCardDescription>Office workbench 状态</DataCardDescription>
          </div>
          <StatusPill tone={plan ? 'warning' : members.length ? 'success' : 'neutral'}>
            {plan ? '草案' : members.length ? '运行中' : '未配置'}
          </StatusPill>
        </DataCardHeader>
        <DataCardMeta>
          <StatusPill tone="neutral" dot={false}>{members.length} members</StatusPill>
          <StatusPill tone={activeTab === 'meeting' ? 'accent' : 'neutral'} dot={false}>{activeTab}</StatusPill>
        </DataCardMeta>
        <DataCardActions className="justify-start">
          <Button type="button" variant="outline" size="sm" onClick={onConfigureTeam}>
            <SlidersHorizontal className="mr-2 h-4 w-4" />
            配置团队
          </Button>
          {plan ? (
            <Button type="button" variant="outline" size="sm" onClick={onApplyPlan} disabled={busy}>
      ...
      <DataCard>
        <DataCardHeader>
          <div>
            <DataCardTitle>Members</DataCardTitle>
            <DataCardDescription>成员、角色和私聊入口</DataCardDescription>
          </div>
          <StatusPill tone="accent">{members.length}</StatusPill>
        </DataCardHeader>
        <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
          {members.slice(0, 8).map((member) => {
            const zone = ZONES[memberZone(member)] || ZONES.generalist;
            return (
              <button
                key={member.agentName}
                type="button"
                className="flex w-full items-center gap-3 rounded-lg border border-border bg-background px-3 py-2 text-left transition-colors hover:bg-muted/40"
                onClick={() => onOpenDirectRoom(member)}
              >
                <SpriteAvatar
                ...
                />
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-semibold">{member.nickname || member.displayName}</span>
                  <span className="block truncate text-xs text-muted-foreground">{zone.label}</span>
                </span>
          ...
          {!members.length ? (
            <EmptyState
              ...
              title="暂无成员"
              description="先运行团队向导或手动组织草案。"
              className="min-h-[180px]"
            />
          ) : null}
        </div>
        <DataCardActions className="justify-start">
          <Button type="button" variant="outline" size="sm" onClick={() => onOpenGroupRoom(members)} disabled={!members.length}>
            <Users className="mr-2 h-4 w-4" />
            开始会议
          </Button>
          <Button asChild variant="outline" size="sm">
            <Link href={dockviewTargets.agents.href} title={dockviewTargets.agents.policy.tabId}>
      ...
      <DataCard>
        <DataCardHeader>
          <div>
            <DataCardTitle>Plan & Memory</DataCardTitle>
            <DataCardDescription>草案、记忆与活动任务</DataCardDescription>
          </div>
          <StatusPill tone={missingZones.length ? 'warning' : 'neutral'}>
            {missingZones.length ? `${missingZones.length} gaps` : 'ready'}
          </StatusPill>
        </DataCardHeader>
        <div className="mt-3 space-y-3 text-sm">
          <div className="rounded-lg border border-border bg-background p-3">
            <div className="font-medium">当前计划</div>
            <p className="mt-1 line-clamp-3 text-xs leading-5 text-muted-foreground">
              {plan?.requirement || '暂无待确认计划。团队向导会生成 clarification、draft 和可应用版本。'}
            </p>
          </div>
          <div className="rounded-lg border border-border bg-background p-3">
            <div className="font-medium">Memory</div>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              {leadMember ? `${leadMember.nickname || leadMember.displayName} 的成员记忆仍由现有成员/Agent 详情能力承载。` : '暂无成员记忆可展示。'}
            </p>
          </div>
          <div className="rounded-lg border border-border bg-background p-3">
            <div className="font-medium">Active task</div>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              {activeRoom ? roomTitle(activeRoom) : recentRooms[0] ? roomTitle(recentRooms[0]) : '暂无活动会议或任务。'}
            </p>
          </div>
        </div>
      </DataCard>
      ...
      <DataCard>
        <DataCardHeader>
          <div>
            <DataCardTitle>Dockview</DataCardTitle>
            <DataCardDescription>打开已有工作区目标</DataCardDescription>
          </div>
        </DataCardHeader>
        <DataCardActions className="mt-3 grid grid-cols-2 justify-stretch">
          <Button asChild variant="outline" size="sm">
            <Link href={dockviewTargets.workflows.href} title={dockviewTargets.workflows.policy.tabId}>Workflows</Link>
          </Button>
          <Button asChild variant="outline" size="sm">
            <Link href={dockviewTargets.agents.href} title={dockviewTargets.agents.policy.tabId}>Agents</Link>
          </Button>
          <Button type="button" variant="outline" size="sm" title={dockviewTargets.chats.policy.tabId} onClick={() => onSetTab('meeting')}>
            Chats
          </Button>
          <Button type="button" variant="outline" size="sm" title={dockviewTargets.organization.policy.tabId} onClick={() => onSetTab('organization')}>
            Org chart
          </Button>
        </DataCardActions>
      </DataCard>
...
export default function OfficePage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const handleApplyCurrentPlan = useCallback(() => {
    if (!plan) return;
    void applyTeamPlan({ plan });
  }, [applyTeamPlan, plan]);
  ...
  const officeDockviewTargets = useMemo<OfficeDockviewTargets>(() => ({
    workflows: {
      href: withOfficeSource('/workflows'),
      policy: resolveDockviewTabPolicy({
        resourceType: 'destination',
        resourceId: 'workflows',
        resourceTitle: 'Workflows',
        resourceSubtitle: 'Opened from Office',
        openMode: 'canonical',
        preferredGroup: 'main',
        contextActions: [{ id: 'open-workflows', label: 'Open workflows', group: 'view' }],
      }),
    },
    agents: {
      href: withOfficeSource('/agents'),
      policy: resolveDockviewTabPolicy({
        resourceType: 'destination',
        resourceId: 'agents',
        resourceTitle: 'Agents',
        resourceSubtitle: 'Opened from Office',
        openMode: 'canonical',
        preferredGroup: 'main',
        contextActions: [{ id: 'open-agents', label: 'Open agents', group: 'view' }],
      }),
    },
    chats: {
      policy: resolveDockviewTabPolicy({
        resourceType: 'office',
        resourceId: 'meeting',
        resourceTitle: 'Office chats',
        openMode: 'canonical',
        preferredGroup: 'main',
        contextActions: [{ id: 'open-chats', label: 'Open chats', group: 'view' }],
      }),
    },
    organization: {
      policy: resolveDockviewTabPolicy({
        ...
        resourceTitle: 'Office organization',
        openMode: 'canonical',
        preferredGroup: 'main',
        dirtyState: Boolean(plan),
        contextActions: [{ id: 'edit-org', label: 'Edit org chart', group: 'edit' }],
      }),
    },
  }), [plan]);
  const officeModeOptions = useMemo<WorkbenchModeOption[]>(() => [
    { value: 'office', label: 'Office' },
    { value: 'meeting', label: 'Meeting', disabled: !teamConfigured },
    { value: 'workflow', label: 'Workflow', disabled: !teamConfigured },
    { value: 'organization', label: 'Organization', disabled: !teamConfigured },
  ], [teamConfigured]);
  const officePrimaryAction = useMemo<WorkbenchAction>(() => ({
    id: 'build-team',
    label: plan ? 'Apply plan' : 'Build team',
    icon: plan ? <CheckCircle2 className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />,
    group: plan ? 'edit' : 'run',
    disabled: busy,
    onSelect: plan ? handleApplyCurrentPlan : handleTeamAction,
  }), [busy, handleApplyCurrentPlan, handleTeamAction, plan]);
  const officeSecondaryActions = useMemo<WorkbenchAction[]>(() => [
    {
      id: 'scope',
      label: 'Scope',
      icon: <UsersRound className="h-4 w-4" />,
      group: 'edit',
      onSelect: () => setAgentScopeOpen(true),
    },
    {
      id: 'start-meeting',
      label: 'Start meeting',
      ...
      onSelect: () => setStatusTab('meeting'),
    },
  ], [teamConfigured]);
  const officeOverflowActions = useMemo<WorkbenchAction[]>(() => [
    {
      id: 'open-workflows',
      label: 'Workflows',
      icon: <Workflow className="h-4 w-4" />,
      group: 'view',
      href: officeDockviewTargets.workflows.href,
    },
    {
      id: 'open-agents',
      label: 'Agents',
      ...
      group: 'view',
      href: officeDockviewTargets.agents.href,
    },
    {
      id: 'edit-org',
      ...
      group: 'edit',
      disabled: !teamConfigured,
      onSelect: () => setStatusTab('organization'),
    },
  ], [officeDockviewTargets.agents.href, officeDockviewTargets.workflows.href, teamConfigured]);


  return (
    <main className={`office-page bg-[#F4F4F1] text-foreground dark:bg-zinc-950 ${embedded ? 'h-full overflow-y-auto' : 'min-h-screen'}`}>
      <div className={`mx-auto flex max-w-[1760px] flex-col ${embedded ? 'min-h-full' : 'min-h-screen'}`}>
        {!embedded ? (
          <header className="flex items-center justify-end border-b border-border bg-[#EDEDE9] px-5 py-3">
            <ThemeToggle />
          </header>
        ) : null}
        ...
        <WorkbenchHeader
          ...
          objectType="Operate"
          mode={statusTab}
          modeOptions={officeModeOptions}
          onModeChange={(mode) => setStatusTab(mode as WorkspaceStatusTab)}
          status={{
            label: plan ? '草案待确认' : teamConfigured ? '已配置' : '待配置',
            tone: plan ? 'warning' : teamConfigured ? 'success' : 'neutral',
          }}
          primaryAction={officePrimaryAction}
          secondaryActions={officeSecondaryActions}
          overflowActions={officeOverflowActions}
          metadata={[
            { label: 'Members', value: officeMembersForDisplay.length },
            { label: 'Mode', value: statusTab },
            { label: 'Dockview', value: officeDockviewTargets.organization.policy.tabId },
          ]}
          className="shrink-0"
        />

        {message ? (
          <div className="mx-5 mt-4 rounded-lg border border-border bg-card px-4 py-2 text-sm text-muted-foreground">
            {message}
          </div>
        ) : null}
        ...
        <div className="grid min-h-0 flex-1 gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="min-w-0 space-y-5">
            ...
            {showTeamConfiguration ? (
              <section className="space-y-4">
                <DataCard className="mx-auto max-w-5xl">
                  <DataCardHeader>
                    <div>
                      <DataCardTitle>团队配置</DataCardTitle>
                      <DataCardDescription>
          ...
          <OfficeInspector
            ...
            dockviewTargets={officeDockviewTargets}
            busy={busy}
            onConfigureTeam={handleTeamAction}
            onApplyPlan={handleApplyCurrentPlan}
            onOpenDirectRoom={openDirectRoom}
            onOpenGroupRoom={openGroupRoom}
            onSetTab={setStatusTab}
          />
        </div>
      </div>
      <AgentScopeDialog
      ...
      <style>{`
        ...
        .org-manager {
          ...
          overflow: hidden;
          border-radius: 14px;
          border: 1px solid rgba(226, 226, 221, 0.96);
          background: rgba(255, 255, 255, 0.96);
          padding: 28px 24px;
          box-shadow: none;
        }
        .dark .org-manager {
          border-color: rgba(226, 226, 221, 0.96);
          background: rgba(255, 255, 255, 0.96);
        }
        .org-reference-title {
          position: relative;
        ...
        .org-connector {
          ...
          height: 48px;
          margin: -2px 7% -10px;
          border-top: 2px solid rgba(139, 92, 246, 0.48);
        }
        ...
        .org-connector::after {
          ...
          inset: 0 2%;
          background:
            linear-gradient(90deg, transparent calc(8.33% - 1px), rgba(139, 92, 246, 0.48) calc(8.33% - 1px) calc(8.33% + 1px), transparent calc(8.33% + 1px)),
            linear-gradient(90deg, transparent calc(25% - 1px), rgba(139, 92, 246, 0.48) calc(25% - 1px) calc(25% + 1px), transparent calc(25% + 1px)),
            linear-gradient(90deg, transparent calc(41.67% - 1px), rgba(139, 92, 246, 0.48) calc(41.67% - 1px) calc(41.67% + 1px), transparent calc(41.67% + 1px)),
            linear-gradient(90deg, transparent calc(58.33% - 1px), rgba(139, 92, 246, 0.48) calc(58.33% - 1px) calc(58.33% + 1px), transparent calc(58.33% + 1px)),
            linear-gradient(90deg, transparent calc(75% - 1px), rgba(139, 92, 246, 0.48) calc(75% - 1px) calc(75% + 1px), transparent calc(75% + 1px)),
            linear-gradient(90deg, transparent calc(91.67% - 1px), rgba(139, 92, 246, 0.48) calc(91.67% - 1px) calc(91.67% + 1px), transparent calc(91.67% + 1px));
        ...
        .org-connector::after {
          ...
          background:
            repeating-linear-gradient(
              ...
              rgba(139, 92, 246, 0.42) calc((100% / max(var(--org-count), 1)) - 2px),
              rgba(139, 92, 246, 0.42) calc(100% / max(var(--org-count), 1))
            );
        ...
        .office-floor {
          ...
          box-shadow:
            ...
            inset 0 16px 44px rgba(255,255,255,0.42);
        }
        .dark .office-floor {
          background:
            linear-gradient(90deg, rgba(255,255,255,0.035) 1px, transparent 1px),
            linear-gradient(0deg, rgba(255,255,255,0.03) 1px, transparent 1px),
            radial-gradient(ellipse at 22% 18%, rgba(59,130,246,0.14), transparent 42%),
            radial-gradient(ellipse at 76% 24%, rgba(139,92,246,0.12), transparent 44%),
            linear-gradient(135deg, #11111A, #14121F 54%, #0D0E14);
          box-shadow:
            inset 0 0 0 1px rgba(255,255,255,0.08),
            inset 0 18px 48px rgba(0,0,0,0.26);
        }
        .office-member-panel {
          position: relative;
          z-index: 2;
        ...
        .station-selected .station-scene {
          filter: drop-shadow(0 12px 16px color-mix(in srgb, var(--zone), transparent 82%));
        }
        .station:focus-visible .station-scene {
        ...
        .station-cubicle-back-shell {
          z-index: 3;
          filter:
            drop-shadow(0 12px 10px rgba(15, 23, 42, 0.1))
        ...
        .station-desk-real {
          ...
          top: 106px;
          z-index: 7;
          filter: drop-shadow(0 10px 12px rgba(15, 23, 42, 0.1));
        }
        .station-lamp-real {
          left: 132px;
          top: 92px;
        ...
        .station-presence-thinking_at_desk .station-agent-sprite {
          filter:
            drop-shadow(0 10px 12px rgba(15, 23, 42, 0.12))
            drop-shadow(0 0 6px color-mix(in srgb, var(--zone), transparent 76%));
        }
        ...
        @media (max-width: 900px) {
          ...
          .office-floor-count-1 {
            min-height: 620px;
          }
          .office-floor-count-1 .station.station-total-1:nth-child(1) {
            left: 50% !important;
            top: 55% !important;
        ...
        @keyframes monitorPulse { 0%,100% { filter: brightness(1) drop-shadow(0 12px 14px rgba(15,23,42,0.12)); } 50% { filter: brightness(1.04) drop-shadow(0 12px 16px rgba(15,23,42,0.16)); } }
        ...
        @keyframes reviewScan { 0%,100% { filter: brightness(1) hue-rotate(0deg) drop-shadow(0 12px 14px rgba(15,23,42,0.12)); } 50% { filter: brightness(1.08) hue-rotate(6deg) drop-shadow(0 12px 18px rgba(15,23,42,0.18)); } }
      `}</style>
    </main>
  );
}
