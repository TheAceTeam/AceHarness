...
async function buildDashboardPromptContext(
  ...
  if (enabledSkills && enabledSkills.length > 0) {
    ...
    const modeRule = creationAssistantEnabled
      ...
      : '按普通工程对话推进，不要使用 workflow creator Skill，也不要输出创建 workflow / Agent 的 `home_sidebar`。';
    skillReferences = `\n\n## 当前启用的 Skills\n\n${refs.join('\n')}\n\n${modeRule}`;
  }

  return envInfo + skillReferences;
}
...
export async function buildDashboardSystemPrompt(
  ...
  return CORE_PROMPT + '\n\n' + LIGHTWEIGHT_WORKFLOW_RULES + context;
}
