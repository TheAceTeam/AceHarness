...
状态：实现计划已完整收敛；代码处于部分实施状态，仍缺 AI 澄清式组队草案、完整组织版本管理、真实办公室行为、direct room 闭环和首页模式入口闭环。
...
- 粗略完成度：基础配置与成员派生约 60%，办公室视觉与交互约 45%，组织架构管理约 40%，AI 组队与确认流程约 20%，记忆运行时统一注入约 35%，direct room 约 10%，首页/模式入口约 20%，整体约 45%-50%。

## 0. 实施总览

### 0.1 当前版本定位

当前 `opc` 分支已经具备“一人公司模式”的可见雏形：Agent YAML 可承载协作空间配置，`/office` 可从 Agent 配置派生成员，用户可以用一句话生成候选团队并确认写回，办公室页面已有组织图、工位、成员面板和基础动画。它适合作为视觉与信息架构验证版本，不适合作为完整产品闭环版本。

下一阶段目标不是继续堆页面装饰，而是补齐产品闭环：

- 组织草案必须从“本地关键词打分”升级为“AI 澄清、AI 草案、用户编辑、确认写回”的正式流程。
- 办公室成员点击必须能创建 direct room，并能继续拉人协作。
- Agent memory 必须在普通对话、会议室、办公室和工作流里通过同一 resolver 控制注入。
- 首页和设置必须真正消费 `workspaceExperience`，完成工程师模式与一人公司模式的入口切换。
- 办公室视觉继续按截图逐层调工位，而不是一次性拼完全部资产。

### 0.2 优先级队列

P0：协作空间 direct room 闭环

- 从 `/office` 成员面板的“私聊”创建或打开 direct room。
- direct room 初始只有一个成员、默认无主题，仍复用现有协作房间和消息机制。
- “拉人协作”把第二个成员加入当前 room，房间自然变成多人会议/办公室协作。
- 房间记录 `spaceType = office | meeting-room` 和 `roomType = direct | meeting`。

P1：组织草案与团队确认闭环

- 新增 `office org draft` 存储和 API。
- 把当前 `/api/office/team/plan` 保留为 fallback。
- 正式生成团队走 `clarify -> draft -> edit -> apply`。
- apply 前只改前端草案和 draft，不写 Agent YAML。
- apply 后写回 `workspaceProfile` 并记录组织版本。

P2：记忆运行时统一

- 所有 Agent 运行入口统一使用 `resolveAgentMemoryContext`。
- 首页 Agent 对话、工作流 Agent、会议室成员、办公室成员都读同一份 `role` memory。
- `agentMemory.runtimeEnabled = false` 时四条路径都不注入长期记忆。
- 日志或调试面板能看到 memory 是否参与。

P3：首页与模式入口

- 首页消费 `workspaceExperience.defaultEntry`。
- 一人公司模式默认进入 `/office`。
- 一人公司模式首页主入口顺序为：办公室、对话、工作流。
- 工程师模式把协作入口文案收敛为“会议室”，保持当前默认体验。

P4：办公室视觉与行为精修

- 固定工作站层继续按“地台 -> 隔间后墙 -> 桌面 -> 设备 -> 椅子/人物 -> 隔间前墙”逐层调。
- 每一层都用截图验收，不再一次性重排全部物件。
- talk、walk、sit 素材按角色批次生成，manifest 固定帧宽、高、脚底锚点和朝向。
- 行为调度从当前模拟状态升级为真实 room / task / workflow 事件驱动。

P5：记忆沉淀候选

- 会议室、办公室、direct room 结束时生成 role memory 候选。
- 候选展示 diff，用户确认后写入 `memory-store`。
- 支持拒绝、回滚、压缩和来源追踪。
...
- 用户可在系统设置选择一人公司模式，并默认进入办公室。
- 用户输入目标后，系统先澄清，再生成组织草案，用户可手动调整成员并确认。
- 确认团队后，Agent YAML 中的 `workspaceProfile` 被准确更新，刷新后办公室成员不丢失。
- 点击任一办公室成员可以进入 direct room，继续拉人后变成办公室会议。
- 开启记忆后，办公室成员发言能读取对应 Agent 的 role memory；关闭后不读取。
- 工程师模式用户默认体验不变，协作入口展示为会议室。
- 办公室工位至少在桌面端不出现明显透视错位、人物倒走、屏幕落地、椅子层级错误等问题。

### 0.4 暂缓项

这些能力不阻塞第一版，但需要保留接口设计：

- 深层记忆数据库和 FTS 检索。
- 组织版本 diff 的完整可视化。
- 拖拽式组织结构编辑。
- 完整 walk/talk/sit 多角色动画素材库。
- 从组织草案直接创建新 Agent 的全自动流程。
...
- [x] 可以在 Agent 编辑页查看、编辑现有 role memory。
- [ ] 清空现有 role memory 的入口需要最终确认。
- [x] 关闭 runtime 开关时 resolver 返回空注入块。
- [x] 开启 runtime 开关时 resolver 可读取 role memory。
- [ ] 现有 Agent 聊天长期角色记忆能力需要完全迁移到统一 resolver 并补回归测试。
- [x] 当前已有 `memory-layers/role/*.yaml` 数据可通过 Agent memory API 读取。
...
- [ ] 首页 Agent 对话复用统一 memory resolver。
- [ ] 工作流 Agent 执行复用统一 memory resolver。
- [ ] 协作空间成员复用统一 memory resolver。
- [ ] 日志中可定位 memory 是否参与。
- [x] resolver 内部支持 `agentMemory.runtimeEnabled` 开关。
- [ ] 关闭 runtime 开关后首页、工作流、协作空间三条路径都不注入。
- [ ] 一人公司模式下 direct room、办公室会议与普通会议室成员都能读取同一份 role memory。

### Phase 3：一人公司 onboarding

当前状态：未完成，系统设置已具备基础字段，首页尚未消费。
...
- [x] 老用户可在系统设置中看到模式切换配置。

### Phase 4：Agent 驱动协作空间成员

当前状态：大部分完成，旧常驻嘉宾迁移仍需确认。
...
- [x] 勾选 `workspaceProfile.residency.office` 后，该 Agent 出现在办公室常驻成员列表。
- [x] 勾选 `workspaceProfile.residency.meetingRoom` 后，该 Agent 可进入会议室推荐来源。
- [ ] 成员发言运行时需要确认使用 Agent 模型、systemPrompt、skills 和 memory。
- [x] 修改昵称只更新 `workspaceProfile.nickname`，Agent `name` 保持稳定。
- [ ] 旧常驻嘉宾数据迁移后能在 Agent 编辑页看到对应配置。

### Phase 4.5：组织架构管理与 AI 组队草案

当前状态：前端雏形完成，正式 AI 草案和版本管理未完成。
...
- [x] 架构图能展示 CEO 核心节点、角色节点和基础组织关系。
- [ ] 架构图岗位详情、职责编辑和汇报关系编辑仍需补齐。
- [x] 用户可以新增、删除、替换成员并形成确认前草案。
- [ ] 用户可以重命名岗位并调整汇报关系。
- [ ] 输入目标后先进入 AI 澄清，再生成组织草案。
- [ ] 每个推荐 Agent 都显示匹配证据、风险和备选候选。
- [x] 用户手动替换岗位 Agent 后，草案和架构图立即更新。
- [x] 当前手动/heuristic 草案确认后才写回 `workspaceProfile` 并更新办公室成员。

### Phase 5：协作空间 direct room

当前状态：未完成，办公室成员面板已有按钮但未打通行为。
...
当前状态：未完成，底层 memory-store 可用，候选审核闭环未接入协作空间。
...
1. 打通 direct room：
   - `/office` 成员面板的“私聊”创建或打开 direct room。
   - direct room 默认无主题。
   - “拉人协作”能把其他成员加入同一 room。
   - 房间记录 `spaceType` 和 `roomType`，旧房间保持兼容。

2. 补齐正式组织草案：
   - 新增 `office org draft` 数据结构和 API。
   - `team/plan` 继续作为 fallback。
   - 正式入口改为 `clarify -> draft -> edit -> apply`。
   - 每个岗位保存 `reportsTo`、职责、候选 Agent、证据、风险和是否空缺。
   - apply 前不写 Agent YAML，apply 后写回 `workspaceProfile`。

3. 统一 memory runtime：
   - 首页 Agent 对话、工作流 Agent、会议室成员、办公室成员都走 `resolveAgentMemoryContext`。
   - 调试信息能看到 memory 是否注入、注入了哪些 scope、是否被截断。
   - `agentMemory.runtimeEnabled` 关闭时四条路径都不注入长期记忆。

4. 完成首页模式入口：
   - 首页读取 `workspaceExperience`。
   - `defaultEntry = office` 时默认进入办公室。
   - 一人公司模式入口顺序改为办公室、对话、工作流。
   - 工程师模式入口文案改为会议室，并保持旧体验。

5. 办公室视觉继续逐层验收：
   - 先锁定地台与隔间。
   - 再锁定桌面遮挡关系。
   - 再锁定显示器、键盘、鼠标、台灯、盆栽和杯子。
   - 最后锁定椅子、人物、标牌和走动/交流动画。
...
- 组织节点拖拽排序和连线编辑。
- 从组织草案自动创建新 Agent。
- 多角色高质量全动作 sprite 资产库。

### 16.3 实施顺序建议

建议按这个顺序推进：

1. direct room：最小闭环最快，能让办公室真正“可对话”。
2. memory resolver 接入协作空间：私聊一旦能跑，马上让它继承 Agent 长期记忆。
3. office org draft：替换当前 heuristic 团队生成逻辑。
4. 首页模式入口：在办公室核心闭环可用后再把默认入口切过去。
5. 视觉与动画精修：随截图反馈持续逐层调，不阻塞功能闭环。
