...
进度：Phase 1 已完成并通过验收（含回归测试）；Phase 1B 待执行；Phase 2 待确认；Phase 3 阻塞于一个待决问题。
...
| 阶段 | 内容 | 风险 | 是否可独立发布 | 状态 |
|---|---|---|---|---|
| Phase 1 | 解除对用户的误伤 + 锁/AI 相关文案 | 低，纯 UI，单文件 | 是 | **已完成** |
| Phase 1B | 第二批反馈：失败态红框裁剪 + 启动对话框默认值 | 低到中，两个文件互不相干 | 是 | 待执行 |
| Phase 2 | 模式改名 + 后果预览 + 隐藏覆盖告知 | 中，跨 3 个界面 + 1 个测试 | 是 | 待确认 |
| Phase 3 | 解耦 `source` 与 `locked` | 高，改核心归一化逻辑 | **需决策，勿自行实施** | 阻塞中 |

Phase 1B 不依赖 Phase 2/3，可以直接做。Phase 2 需确认后再做，Phase 3 禁止在未获明确指示时动手。
