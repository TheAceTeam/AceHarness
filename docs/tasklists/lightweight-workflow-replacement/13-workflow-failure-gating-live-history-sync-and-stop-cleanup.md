...
Status: In Review (99%; current focused revalidation passed, Linux integration and shared gate pending)
...
- `2026-07-31 current-gate regression`: a mocked UUID collision let two parallel steps overwrite one persisted step log. Step-log allocation now guards collisions for ordinary and child-workflow steps. The focused manager suite passed `123/123`; the final current full gate also passed.
- `2026-08-01 current revalidation`: a scoped history status request could fall back to another same-config in-memory manager when its exact run record was unavailable, which could contaminate overview/state/live output with another run. Both compact and normal status resolvers now return target-scoped idle/not-found data instead. `tests/api-workflow-status-route.test.ts` covers both responses; the direct Task 13 suite passed `6 files / 192 tests` (`state-machine-workflow-manager`, recovery routes, Workbench history sync, status route, process manager, and stop route). Linux real ACPX process-tree proof and shared final gate remain open.
- `2026-08-01 Linux integration`: WSL2 Ubuntu 24.04 ran `tests/process-manager.linux-process-tree.integration.test.ts` against the production Linux `ps`/subtree/signal path. Two real Node parent/child trees were registered through isolated ACP-shaped records; cleanup matched and terminated only target record `record-run-a` (`914233/914247`) while `record-run-b` (`914234/914248`) remained alive. The integration passed `1/1`. This proves the process-manager Linux ownership path, not a live ACPX provider agent: ACPX had no usable Linux session/CLI in this environment. That narrower provider-specific proof remains open.
