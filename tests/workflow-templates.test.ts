...
describe('workflow templates', () => {
  test('accepts legacy ACE manifests but normalizes the public contract to CSIHarness', async () => {
    const { workflowTemplateManifestSchema } = await import('@/lib/workflow-template/types');
    ...
    expect(manifest.apiVersion).toBe('csiharness.io/v1alpha1');
    expect(manifest.spec.compatibility).toEqual({ csiharness: '^1.0.0' });
  });
  ...
  test('saves an existing workflow as a sanitized private template', async () => {
    await withIsolatedAceHome(async (aceHome) => {
      await withTempWorkspace(async ({ workspace }) => {
        ...
        const templateManifest = parse(await readFile(
          path.join(path.dirname(templateWorkflowPath), 'manifest.yaml'),
          'utf8',
        ));
        expect(templateManifest.apiVersion).toBe('csiharness.io/v1alpha1');
        expect(templateManifest.spec.compatibility).toEqual({
          csiharness: '>=0.1.0 <1.0.0',
        });
