...
满足这些条件时，先在 `implementation-summary.md` 写入待决检查点，再使用 CSIHarness 运行时提供的步骤内人工客服协议请求一次性授权。问题正文说明已知事实、精确范围、必要性、已检查的替代方案和影响；每轮一个请求，发出后停止当前步骤。协议输出由 CSIHarness 运行时定义，本 Skill 只维护触发条件和问题内容。
