...
description: >-
  ...
  支持 ACEHarness 工作流前序步骤产出与 review_tmp 蓝军产物；可运行于任意 Agent 环境。
---
...
| **WORK_DIR** | 黄军产出目录，默认 `judge_tmp`（写 `judge_context.json`、`feedbacks.json`） |
| **脚本** | `prepare_judge_context.py`（整理输入）、`report_feedback.py`（上报） |
...
| **时间戳详情文件** | `{ISO时间}-{阶段/状态}-{步骤}.md` | 阶段模式完整 raw 输出（优先于摘要文件） |
| **状态机同状态** | 文件名形如 `实施-代码审查.md` | 上一步蓝军在同状态内的归档（Judge prompt 未必自动注入全文，需 Read） |
| **阶段模式** | 文件名形如 `代码审查.md` | 同迭代内已完成蓝军步骤产出 |

工作流 `{runId}` 与 outputs 绝对路径见当前步骤 Prompt 的「文档输出要求」或前序步骤中的 `` `.../outputs/...` ``。`prepare_judge_context.py` 可从 `--prompt-context-file` 自动解析该路径。
...
- `issues` 为空且 `judge_context.json` 中 `blue_review_confirmed_empty=true` → 可 `feedbacks: []`，verdict `pass`
...
```mermaid
flowchart LR
  A[prepare_judge_context / 手工加载] --> B[四步复核]
  B --> C[feedbacks.json]
...
若已知 Prompt 全文已保存，可附加 `--prompt-context-file judge_tmp/current_prompt.txt` 以自动解析 `runs/.../outputs` 路径。

脚本写出 `judge_tmp/judge_context.json`（含 `issues`、`blue_team_outputs`、`review_artifacts`、`warnings`、`blue_review_confirmed_empty`）。**以 stdout 打印的路径为准。**
...
`feedbacks: []` → 跳过上报（蓝军零 issues 且已确认时允许）。
...
**防重复上报**：上报成功后脚本自动**刷新** `feedbacks.json`——全部成功则清空为 `{"summary":"","feedbacks":[]}`；部分失败则仅保留未成功条目供重试。
...
```
输入
[ ] prepare_judge_context.py（或等价手工加载）
[ ] Read judge_context.json
[ ] 读 review_tmp/*、规则卡片、issue 源文件
[ ] diff 不足时用 git / Read 补证据

复核
[ ] judge_prompt.md 四步复核每条 issue
[ ] 覆盖写入 judge_tmp/feedbacks.json（agent: true）

上报与工作流
[ ] report_feedback.py judge_tmp/feedbacks.json
[ ] 输出 verdict JSON 与 <step-conclusion>（状态末步）
[ ] 向用户汇报 OK/WARN
```
...
| `prepare_judge_context.py` | 汇总 ACEHarness / review_tmp 输入 |
