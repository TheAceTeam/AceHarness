...
const logFile = resolve(process.env.BRIDGE_LOG_FILE || '.csiharness/wechat-bridge-relay.ndjson');

if (!aceWebhookUrl || !aceSecret) {
  console.error('Missing CSIHARNESS_WEBHOOK_URL or CSIHARNESS_SECRET.');
  console.error('Example: CSIHARNESS_WEBHOOK_URL=http://127.0.0.1:3001/api/channels/inbound/channel-xxx CSIHARNESS_SECRET=xxxx node scripts/wechat-bridge-relay.mjs');
  process.exit(1);
}
